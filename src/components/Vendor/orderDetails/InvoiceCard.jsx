...
const InvoiceCard = React.forwardRef(({ orderData }, ref) => {
  const { id, product, orderInfo, customer, vendor } = orderData;
  const total = Number(orderInfo.price.replace(/[^\d]/g, "")) || 28999;
  const base = Math.round(total / 1.18);
  const gst = total - base;

  // Format vendor address - handle both string and object formats
  const formatVendorAddress = (address) => {
    if (!address) return '0 Subash Nagar Phase II, Sarojini Nagar, Lucknow- 226008, Uttar Pradesh';
    if (typeof address === 'string') return address;
    if (typeof address === 'object') {
      const parts = [address.line1, address.line2].filter(Boolean);
      return parts.length > 0 ? parts.join(', ') : '0 Subash Nagar Phase II, Sarojini Nagar, Lucknow- 226008, Uttar Pradesh';
    }
    return '0 Subash Nagar Phase II, Sarojini Nagar, Lucknow- 226008, Uttar Pradesh';
  };

  const vendorAddress = formatVendorAddress(vendor?.address);
  return (
    <div ref={ref} className="bg-white p-5 border border-gray-200 text-gray-800 text-[10px] w-full max-w-[620px] mx-auto font-sans leading-relaxed" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-gray-900 text-xs">{vendor?.companyName || vendor?.name || 'IERADA FASHION PRIVATE LIMITED'}</h1>
            <p className="text-gray-500 text-[8px]">{vendorAddress}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-3 text-[9px] pb-2 border-b">
        <div>
          {[`Company Name: ${vendor?.companyName || vendor?.name || 'Ierada Fashion Pvt Ltd'}`, `Website: ${vendor?.website || 'www.ierada.com'}`, `Address: ${vendorAddress}`, vendor?.gstin ? `GSTIN: ${vendor.gstin}` : null].filter(Boolean).map((x, i) => <p key={i}>{x}</p>)}
        </div>
